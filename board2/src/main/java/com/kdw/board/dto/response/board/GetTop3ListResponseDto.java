...
@ApiModel(value = "좋아요 기준 상위 3개 게시물 리스트 가져오기 Response Data")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class GetTop3ListResponseDto {
    ...
    @ApiModelProperty(value = "조회수", example = "4", required = true)
    private int viewCount;

    @ApiModelProperty(value = "작성자 닉네임", example = "KDW", required = true)
    private String writerNickname;

    @ApiModelProperty(value = "작성자 프로필 이미지 URL", example = "http://~", required = false)
    private String writerProfileUrl;

    @ApiModelProperty(value = "댓글 수", example = "22", required = true)
    private int commentCount;

    @ApiModelProperty(value = "좋아요 수", example = "99", required = true)
